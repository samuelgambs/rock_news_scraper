...
class BraveWordsScraper(BaseScraper):
    """
    Scraper para coletar notícias do site Brave Words via RSS.
    """
    def __init__(self):
        super().__init__("https://bravewords.com")
        self.source = "Brave Words"
        self.storage = NewsStorage()

    def fetch_articles(self, limit=10):
        """Busca as últimas notícias do feed RSS e armazena as novas."""
        feed_url = "https://bravewords.com/feed/"
        news_list = []
    ...
    def fetch_article_content(self, url):
        ...
        content_div = soup.find('div', class_='post-content')  # Ajuste para Brave Words
        ...
        if content_div:
            ...
            for p in paragraphs:
                ...
                text = re.sub(r'(?<=[a-z])(?=[A-Z])', ' ', text)  # Insere espaço entre palavras coladas corretamente
